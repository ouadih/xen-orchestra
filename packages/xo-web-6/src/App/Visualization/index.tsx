...
export default class Visualization extends Component<any, any> {
  ...
  fetchVmStats1 = () => {
    xoCall('vm.stats', { id: '28851ef6-951c-08bc-a5be-8898e2a31b7a', granularity: this.state.granularity }).then(
      ({ endTimestamp, stats: { cpus }, interval, stats: { memory }, stats: { xvds }, stats: { vifs } }) => {
        ...
        this.state.maxVifVM1 = Math.max(this.state.maxVifTxVM1, this.state.maxVifRxVM1)
        this.state.maxVVM1 = Math.max(this.state.maxVTxVM1, this.state.maxVRxVM1)
  ...
  fetchVmStats2 = () => {
    xoCall('vm.stats', { id: '8c05611a-0f25-948e-dc1a-49329576866d', granularity: this.state.granularity }).then(
      ({ endTimestamp, stats: { vifs }, stats: { cpus }, stats: { xvds } }) => {
        ...
        this.state.maxVifVM2 = Math.max(this.state.maxVifTxVM2, this.state.maxVifRxVM2)
        this.state.maxVVM2 = Math.max(this.state.maxVTxVM2, this.state.maxVRxVM2)
  ...
  render() {
    ...
    return (
      ...
      <div>
        ...
        <VuCpuStatsVM1 dataXvds={this.state.dataXvds} propXvds={this.state.propXvds} propXvdsR={this.state.propXvdsR} maxVif={this.state.maxVif} />
        <VuCpuStatsVM2 dataXvds2={this.state.dataXvds2} propXvds2={this.state.propXvds2} propXvdsR2={this.state.propXvdsR2} maxVif={this.state.maxVif} />
        <VuCpuStatsssVM1 dataCpuVm1={this.state.dataCpuVm1} propCpusVm1={this.state.propCpusVm1} />
        <VuCpuStatsssVM2 dataCpuVm2={this.state.dataCpuVm2} propCpusVm2={this.state.propCpusVm2} />
        <VuVifsStatsVM1 dataVifs={this.state.dataVifs} propVifs={this.state.propVifs} propVifsR={this.state.propVifsR} maxV={this.state.maxV} />

        <VuVifStatsVM2 dataVifs2={this.state.dataVifs2} propVifs2={this.state.propVifs2} propVifsR2={this.state.propVifsR2} maxV={this.state.maxV} />
      ...
      <div>
        ...
        <div>
          ...
          >
            ...
            {
              (this.props.propXvds
                .map((currProperty: any) => `xvds_w_${currProperty}`).map((property: any, index: any) => <Area connectNulls isAnimationActive={false}
                  type="monotone" dataKey={property} stroke={allColors[index]} fill={allColors[index]} />))
            }
            {
              (this.props.propXvdsR.map((currProperty: any) =>
                `xvds_r_${currProperty}`).map((property: any, index: any) => <Area connectNulls isAnimationActive={false}
      ...
      <div>
        ...
        <div>
          ...
          >
            ...
            {
              (this.props.propXvdsR2.map((currProperty: any) =>
                `xvds_r_${currProperty}`).map((property: any, index: any) => <Area connectNulls isAnimationActive={false}
                  type="monotone" dataKey={property} stroke={colors[index]} fill={colors[index]} />))
            }
          </AreaChart>
      ...
      <div>
        ...
        <div>
          ...
          >
            ...
            {
              (this.props.propVifsR.map((currProperty: any) =>
      ...
      <div>
        ...
        <div>
          ...
          >
            ...
            {
              (this.props.propVifsR2.map((currProperty: any) =>
                `vifs_rx_${currProperty}`).map((property: any, index: any) => <Area connectNulls isAnimationActive={false}
                  type="monotone" dataKey={property} stroke={colors[index]} fill={colors[index]} />))
            }
          </AreaChart>
